...
`fish`, `xonsh`, and `zsh` support for the `nix develop`, `nix shell`, `nix run`, and `nix-shell`
environments of the Nix package manager.

Features:

* When entering a `nix develop`, `nix shell`, `nix run`, or `nix-shell` environment, the shell stays the same.
